...
public class PatientController {
    ...
    /**
     * 1. Retrieve patients with pagination support.
     * 2. Return a Page of patients with a 200 OK status.
     */
    @GetMapping("/paged")
    public ResponseEntity<Page<Patient>> getAllPaged(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "id") String sortBy,
            @RequestParam(defaultValue = "asc") String sortDir) {
        logger.info("Received request to list patients with pagination: page={}, size={}, sortBy={}, sortDir={}",
                page, size, sortBy, sortDir);
        Sort sort = sortDir.equalsIgnoreCase("desc") ? Sort.by(sortBy).descending() : Sort.by(sortBy).ascending();
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<Patient> patients = patientService.getAllPaged(pageable);
        logger.debug("Returning page {} with {} patients out of {} total",
                page, patients.getNumberOfElements(), patients.getTotalElements());
        return ResponseEntity.ok(patients);
    }
